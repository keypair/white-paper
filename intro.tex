...
\ref{sect:channels}),
an off-chain encrypted peer-to-peer communication protocol for trustless execution of smart contracts. After
...
Further increase increase of throughput is achieved by using a novel consensus algorithm.
...
is a throughput of about 117 public mainnet on-chain transactions per second~\cite{source?} with a (micro)blocktime of three seconds and with
low latency as opposed to the seven transactions per second of Bitcoin together with the ten minutes block confirmation time. 

Additionally to the PoW, a weighted delegated governance system has been
implemented. This way the users of \blockchain\ can express their (hardfork)choice in a tamperproof-way, signaling to the miners which fork would be more attractive in the future. 
